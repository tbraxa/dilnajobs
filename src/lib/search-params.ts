...
function one(input: Record<string, string | string[] | undefined>, key: string): string | undefined {
  const v = input[key];
  if (Array.isArray(v)) {
    return [...v].reverse().find((item) => typeof item === "string" && item.trim()) || undefined;
  }
...
export function parseSearch(input: Record<string, string | string[] | undefined>): SearchQuery {
  const raw = {
    ...
    city: one(input, "city"),
    salaryMin: one(input, "salaryMin") ?? one(input, "salaryStep") ?? one(input, "mzda") ?? one(input, "mzda_od"),
...
/** Drawer facets on Upravit filtry. Query lives in the search bar; place is also a chip. */
