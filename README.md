...
**Provozní checklist pro fairjobs.cz**

1. Merge tohoto PR do `main` (PR #1 s MVP/admin už je sloučený).
2. Udržovat DNS `fairjobs.cz` (+ `www`) nasměrované na Vercel nebo Cloud Run.
3. Účet [Resend](https://resend.com): ověřit doménu, `RESEND_API_KEY`, `EMAIL_FROM`.
4. Účet [Stripe](https://stripe.com) (sandbox `sk_test_` / `whsec_`, webhook `https://fairjobs.cz/api/stripe/webhook` → `checkout.session.completed`).
