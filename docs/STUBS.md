...
| **ARES IČO** | Live GET `ares.gov.cz/.../ekonomicke-subjekty/{ico}` with timeout (`ARES_TIMEOUT_MS`, default 4s). Checksum-invalid IČO is rejected. Timeout/5xx → save company as `pending` / `manual` and continue. `ARES_DISABLED=true` skips HTTP (checksum only, status `pending`). | Keep live call; tighten inactive-subject mapping and caching. |
