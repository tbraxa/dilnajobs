...
| **ARES IČO** | Register still accepts a well-formed IČO without blocking on HTTP. **Načíst z ARES** calls `GET /api/ares` (public ARES REST). `verification_status` stays `pending` until ops marks verified. | Ops verification queue. |
