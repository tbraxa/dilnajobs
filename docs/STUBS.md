...
| **Stripe** | Missing either Stripe secret or webhook secret → paid checkout is disabled and no order is created. | Set both `STRIPE_SECRET_KEY` + `STRIPE_WEBHOOK_SECRET`. Endpoint: `POST /api/stripe/webhook`; signed success events fulfill the exact order amount. |
