...
| **First-post review** | New jobs from the portal are `pending_review`. Seed jobs are mostly `published` plus one pending row. Moderate at `/admin/jobs`. `FEATURE_AUTO_PUBLISH_FIRST_JOB` is a kill-switch, default false. | Leave the flag false in production. |
...
| **pg-boss worker** | Expiry can be run with `npm run worker` (writes `system_heartbeats.job_expiry`). Catalog also hides `expires_at < now()`. | Cron or always-on worker in deploy. |
